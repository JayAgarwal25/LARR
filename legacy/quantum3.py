...
from numpy import trapezoid  # Use trapezoid instead of deprecated trapz
...
def compute_M_q_numeric(tr, omega_x_au, kL_tr, q0, Z,
    ...
    return trapezoid(integrand_r, r_grid)
...
def DP_single(omega_x_eV, t_grid, E_t, kL_t, q_t,
    ...
    n_t = len(t_grid)
    print(f"  Computing M(t) for ω_x={omega_x_eV:.1f} eV ({n_t} time points)...", flush=True)
    
    for i, t_val in enumerate(t_grid):
        if i % 50 == 0:  # Progress update every 50 points
            print(f"    Progress: {i}/{n_t} ({100*i/n_t:.1f}%)", flush=True)
        kL_val = kL_t[i]
...
if __name__ == "__main__":
    ...
    for row, nc in enumerate(nc_list):
        for col, phi in enumerate(phi_list_deg):
            ...
            # Use much coarser time grid for DP - it's very expensive!
            DP_vals = compute_DP_parallel(nc, phi, omega_x_eV_grid,
                                         Nr=80, Rmax=60.0, Nmu=60, t_nt=300)
