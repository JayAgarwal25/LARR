...
if __name__ == "__main__":
    # DRASTICALLY reduced grid for testing
    omega_x_eV_grid = np.linspace(0.0, 300.0, 100)  # Only 10 points for testing!
    
    print(f"\n{'#'*60}")
    print("WARNING: DP calculation is VERY expensive!")
    print(f"Testing with only {len(omega_x_eV_grid)} ω_x points")
    print("Estimated time per ω_x: 2-10 minutes")
    print(f"Total estimated time: {len(omega_x_eV_grid)*5*len(nc_list)*len(phi_list_deg)/60:.1f} hours")
    print(f"{'#'*60}\n")
    ...
    for nc in nc_list_test:
        for phi in phi_list_test:
            ...
            DP_vals = compute_DP_serial(nc, phi, omega_x_eV_grid,
                                       Nr=40, Rmax=40.0, Nmu=30, t_nt=500)
