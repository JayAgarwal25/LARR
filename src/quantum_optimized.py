...
if __name__ == "__main__":
    ...
    # [Setup code remains the same as your original...]
    ...
    # --- RUN CDP ---
    cdp_csv = f"results_fast/CDP_nc{single_nc}_phi{int(single_phi)}_{timestamp}.csv"
    # Create args list (Only scalars!)
    args_cdp = [(om, Nr, R_MAX_SIM, Nmu) for om in omega_x_eV_grid]
    
    t0 = time.time()
    CDP_vals = run_parallel_computation_safe(
        "CDP", args_cdp, 
        t_grid, E_t, kL_t, None, omega_t, # Pass arrays needed for CDP
        cdp_csv
    )
    print(f"[time] CDP took {(time.time()-t0)/60:.2f} min")

    # --- RUN DP ---
    dp_csv = f"results_fast/DP_nc{single_nc}_phi{int(single_phi)}_{timestamp}.csv"
    # Create args list (Only scalars!)
    args_dp = [(om, Nr, R_MAX_SIM, Nmu) for om in omega_x_eV_grid]
    
    t0 = time.time()
    DP_vals = run_parallel_computation_safe(
        "DP", args_dp, 
        t_grid, None, kL_t, None, None, # Pass arrays needed for DP
        dp_csv
    )
    print(f"[time] DP took {(time.time()-t0)/60:.2f} min")

    # --- PLOT (Using the returned arrays) ---
    plt.figure(figsize=(7,5))
    plt.plot(omega_x_eV_grid, np.maximum(CDP_vals, 1e-50), label='CDP', linewidth=3.0, color='blue')
    plt.plot(omega_x_eV_grid, np.maximum(DP_vals, 1e-50),  label='DP', color='red')
    plt.yscale('log'); plt.xlim(0, 500); plt.ylim(1e-18, 1e-3)
    plt.xlabel("ωₓ (eV)"); plt.ylabel("Probability (a.u.)")
    plt.legend(); plt.tight_layout()
    plt.savefig(f"DP_vs_CDP_{timestamp}.png", dpi=180)
    print(f"Saved plot: DP_vs_CDP_{timestamp}.png")
